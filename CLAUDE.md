...
- **PDF branding** (`_AssessmentPDF(FPDF)` in `main.py`): The PDF's `header()`/`footer()` draw a colored banner (logo from `backend/assets/ats-logo.jpg` in a white box, title, "Summary Report" subtitle) and a page-number footer on every page — `accent_color` is set from `THEME_COLORS[app_config["theme"]]` right before `add_page()`, so the exported report always matches whatever theme is currently active in the admin panel (switching the app theme also re-colors future PDF exports, no separate PDF-specific setting). The Q&A body uses fpdf2's `pdf.table()` API (a real bordered grid with a colored heading row) instead of the earlier stacked `multi_cell()` blocks — simpler and avoids the manual `set_x(pdf.l_margin)` dance the old version needed.
