...
- **Assessment PDF export** (`GET /chat/assessment/pdf` in `main.py`): Once an assessment completes, its real answers are saved into `completed_assessments[username]` (separate from `assessment_states`, which gets cleared immediately) so this route can render an accurate PDF on demand via `fpdf2` (`_build_assessment_pdf()`) — built directly from the same server-side data used for the chat summary, not from the model. Auth is the normal JWT (any logged-in user), not the MCP key. Returns 404 if the user has no completed assessment. `fpdf2` gotcha: `multi_cell()`'s cursor X position doesn't reliably reset between calls, so each call explicitly does `pdf.set_x(pdf.l_margin)` first and passes `new_x="LMARGIN", new_y="NEXT"` — omitting this caused `FPDFException: Not enough horizontal space to render a single character`. `POST /chat/clear` also clears `completed_assessments`.
- **Assessment SSE signaling** (`main.py`/`Chat.tsx`): Every `/chat/message` SSE stream ends with `data: {"in_assessment": bool, "assessment_completed": bool}` before `[DONE]`, so the frontend knows without guessing whether the user is mid-assessment. `Chat.tsx` uses this to swap the input placeholder to "Answer the question above..." during the flow, and to show a "View PDF Summary" button once `assessment_completed` fires. Both reset on "New conversation."
- **PDF preview modal** (`Chat.tsx`): "View PDF Summary" fetches `/chat/assessment/pdf` as a blob (needs the JWT header, so a plain `<a href>` won't work) and renders it inline via `<iframe src={blobUrl}>` in a modal overlay, using the browser's native PDF viewer — no auto-download. A separate `<a download>` inside the modal lets the user explicitly save it. The object URL is revoked on modal close and on "New conversation" to avoid leaking memory. Note: headless-Chromium's stripped-down "headless shell" build (used by some test harnesses) doesn't include the PDF viewer plugin and renders this blank — the full Chromium/Chrome/Edge/Firefox binaries a real user runs all support it natively.
